...
# --------------------------------------------------------------
# Async IO writer class. This class spawns a background thread that
# writes lines from a queue into a file.
class AsyncWriter:
...
# --------------------------------------------------------------
# IKDataWriter now uses AsyncWriter instead of doing file I/O inline.
class IKDataWriter:
    ...
    def write_data(
        ...
        # Offload IO by writing the JSON line in a background thread.
...
# In RobotDataWorker, we add an AsyncWriter for robot_data.
class RobotDataWorker:
    def __init__(
        self, dirname, stop_event, start_event, h1_shm_array, teleop_shm_queue
    ):
        ...
        self.robot_data_writer = None  # Will be created in run_session
    ...
    def get_robot_data(self, color_frame, depth_frame, time_curr):
        ...
        # Do not write files here; just prepare the data dictionary.
        with self.h1_lock:
        ...
        logger.debug("worker: finish getting robot data")
        return robot_data
    ...
    def run_session(self):
        # Create an async writer for robot data
            ...
            while not self.stop_event.is_set():
                ...
                if time_curr <= next_capture_time:
                    ...
                    if "last_robot_data" in locals() and last_robot_data is not None:
                        ...
                        self.robot_data_writer.write(json.dumps(last_robot_data))
                    else:
                        logger.error(
    ...
    def reset(self):
        # Fully reset the worker’s state.
        self.frame_idx = 0
        # Reinitialize ZeroMQ context, socket, and teleoperator:
        self.context = zmq.Context()
        self.socket = self.context.socket(zmq.PULL)
        self.socket.connect("tcp://192.168.123.162:5556")
        self.socket.setsockopt(zmq.RCVTIMEO, 200)
...
class RobotTaskmaster:
    ...
    def start(self):
        ...
        while not self.stop_event.is_set():
            ...
            logger.debug("robot_master: tv finish")
    ...
    def reset(self):
        ...
        if self.running:
            self.stop()
        # Instead of just clearing the event, reinitialize it if needed.
        self.stop_event.clear()
        self.h1hand.reset()
        self.h1arm.reset()
        self.first = True
        self.running = False
        # Do not recreate shared memory; just zero its content.
        self.h1_shm_array[:] = 0
        self.dirname = time.strftime(f"demos/{self.task_name}/%Y%m%d_%H%M%S")
...
if __name__ == "__main__":
    ...
    def run_dataworker(
        dirname, stop_event, start_event, h1_shm_array, teleop_shm_queue
    ):
    ...
    try:
        while True:
            ...
            if user_input == "s" and not taskmaster.running:
                ...
                taskmaster.reset()
                # Restart the robot data worker process after reset.
                robot_data_proc = create_robot_data_proc(taskmaster)
                robot_data_proc.start()
